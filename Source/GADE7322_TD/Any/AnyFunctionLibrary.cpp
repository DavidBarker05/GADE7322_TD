...
FAny UAnyFunctionLibrary::MakeAny(const int32& Value)
{
    checkNoEntry()
    return {};
}
...
DEFINE_FUNCTION(UAnyFunctionLibrary::execMakeAny)
{
    ...
    if (!bSuccess)
    {
        FBlueprintExceptionInfo ExceptionInfo(EBlueprintExceptionType::AbortExecution,
                                              LOCTEXT("AnyMake_MakeInvalidValueWarning",
                                                      "Failed to resolve the Value for Make Any"));
...
DEFINE_FUNCTION(UAnyFunctionLibrary::execSetAny)
{
    ...
    {
        FBlueprintExceptionInfo ExceptionInfo(EBlueprintExceptionType::AbortExecution,
                                              LOCTEXT("InstancedStruct_SetInvalidValueWarning",
...
DEFINE_FUNCTION(UAnyFunctionLibrary::execToAny)
{
    ...
    if (!bSuccess)
    {
        FBlueprintExceptionInfo ExceptionInfo(EBlueprintExceptionType::AbortExecution,
                                              LOCTEXT("InstancedStruct_SetInvalidValueWarning",
                                                      "Failed to resolve the Value for To Any"));
        FBlueprintCoreDelegates::ThrowScriptException(P_THIS, Stack, ExceptionInfo);
        P_NATIVE_BEGIN;
            *(FAny*)RESULT_PARAM = FAny();
        P_NATIVE_END;
